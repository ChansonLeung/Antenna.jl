...
rotate_pattern = (pattern::anten_pattern, coord::Matrix{Float64}) -> begin
    ...
    # plot_E_vec_field_test(Gθ_grid)

    Gϕ_grid = [begin
        θ,ϕ = rotate_vec_in_sph(θ,ϕ,coord');
        (coord*vec_ϕ(θ,ϕ))*pattern.ϕ(θ,ϕ) end
    for (θ,ϕ) = zip(θ_grid, ϕ_grid)]
    # plot_E_vec_field_test(Gϕ_grid)
    
    vec_θ₁_map_grid = [begin
    vec_θ(θ,ϕ)end 
    for (θ,ϕ) = zip(θ_grid, ϕ_grid)]
    # plot_E_vec_field_test(vec_θ₁_map_grid)
    vec_ϕ₁_map_grid = [begin 
        vec_ϕ(θ,ϕ) end
        for (θ,ϕ) = zip(θ_grid, ϕ_grid)]
    # plot_E_vec_field_test(vec_ϕ₁_map_grid)
    ...
    # vec_vec_globe = [
    #     vec_θ(θ, ϕ) * result.θ(θ, ϕ) .+ vec_ϕ(θ, ϕ) * result.ϕ(θ, ϕ)
    #     for (θ,ϕ) = zip(θ_grid, ϕ_grid)
    # ]
...
function cal_pattern(point::Vector{anten_point}, θₜ, ϕₜ, k = k, θ = θ_default, ϕ = ϕ_default)
    ...
    Threads.@threads for (ind_point, point) = collect(enumerate(point))
        for (i, θ) = enumerate(θ), (j, ϕ) = enumerate(ϕ)
            result_θ[i, j, ind_point] = Pi(point.p, point.pattern.θ, θ, ϕ, θₜ, ϕₜ, k)
            result_ϕ[i, j, ind_point] = Pi(point.p, point.pattern.ϕ, θ, ϕ, θₜ, ϕₜ, k)
        end
    end
