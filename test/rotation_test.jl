...
@time rotate_pattern!(dipole_hfss[1], RotZ(pi/4)*RotY(pi/4)|>Matrix);

dipole = read_hfss_pattern("test/dataset/rE_theta_phi_dipole_rotate.csv")

@time dipole.θ.(θ_grid, ϕ_grid);
@btime dipole.ϕ.(θ_grid, ϕ_grid);
...
# cal_pattern()
# contour(dipole_hfss[1], fill=true)